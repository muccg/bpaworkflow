...
DEFAULT_FROM_EMAIL = env.get(
    "DJANGO_DEFAULT_FROM_EMAIL", "Bioplaforms Data Portal <help@bioplatforms.com>"
)
EMAIL_SUBJECT_PREFIX = env.get(
    "DJANGO_EMAIL_SUBJECT_PREFIX", "[Bioplatforms Workflow] "
)
...
SESSION_COOKIE_HTTPONLY = SESSION_COOKIE_HTTPONLY = env.get(
    "session_cookie_httponly", True
)
SESSION_COOKIE_SECURE = env.get("session_cookie_secure", PRODUCTION)
SESSION_COOKIE_NAME = env.get(
    "session_cookie_name", "ccg_{0}".format(SCRIPT_NAME.replace("/", ""))
)
...
TEMPLATES = [
    {
        ...
        "DIRS": [os.path.join(WEBAPP_ROOT, "bpaworkflow", "templates")],
        "APP_DIRS": False,
        "OPTIONS": {
            "context_processors": [
...
LOGGING = {
    ...
    "loggers": {
        ...
        "bpaworkflow.bpaworkflow.management.commands": {
            "handlers": ["rainbow"],
            "level": "DEBUG",
            "propagate": False,
        },
...
if env.get("DEBUG_TOOLBAR", False):
    ...
    INTERNAL_IPS = (
        "127.0.0.1",
        "172.16.2.189",
    )  # explicitly set this for your environment
...
CKAN_CHECK_PERMISSIONS_URL = env.get(
    "ckan_check_permissions_url", "/user/private/api/bpa/check_permissions"
)
...
CKAN_AUTH_TOKEN_HEADER_NAME = env.get(
    "ckan_auth_token_header_name", "HTTP_X_BPAWORKFLOW_CKAN_TOKEN"
)
