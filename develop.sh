...
: "${CCG_DOCKER_ORG:=bioplatformsaustralia}"
: "${BIOPLATFORMS_COMPOSER:=bioplatforms-composer}"
: "${BIOPLATFORMS_COMPOSER_VERSION:=latest}"
: "${CCG_PIP_PROXY=0}"
: "${CCG_HTTP_PROXY=0}"

export CCG_DOCKER_ORG BIOPLATFORMS_COMPOSER BIOPLATFORMS_COMPOSER_VERSION CCG_PIP_PROXY CCG_HTTP_PROXY
...
docker run --rm ${TTY_OPTS} ${ENV_FILE_OPT} \
    ...
    -w "$(pwd)" \
    "${CCG_DOCKER_ORG}"/"${BIOPLATFORMS_COMPOSER}":"${BIOPLATFORMS_COMPOSER_VERSION}" \
    "$@"
