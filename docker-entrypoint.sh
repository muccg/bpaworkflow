...
if [ "$1" = 'aloe' ]; then
    ...
    cd /app/bpaworkflow || exit
    _aloe "$@"
fi

warn "[RUN]: Builtin command not provided [tarball|aloe|runtests|runserver|runserver_plus|uwsgi|gunicorn|uwsgi_local]"
info "[RUN]: $*"
